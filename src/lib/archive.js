...
module.exports = function (state, bus) {
  ...
  bus.once('dweb', function () {
    ...
    if (state.dweb.archive.content) return bus.emit('archive:content')
    state.dweb.archive.once('content', function () {
...
function selectiveSync (state, bus) {
  var archive = state.dweb.archive
  ...
  if (state.opts.empty) {
    archive.metadata.update(function () {
      return bus.emit('exit:warn', `dWeb archive successfully created in empty mode. Download files using pull or sync.`)
    })
  }
