module.exports = {
  ...
  command: status,
  help: [
    'Get information on about the DWeb in a directory.',
    '',
    'Usage: dweb status'
  ].join('\n'),
  options: []
...
function status (opts) {
  ...
  neat.use(function (state, bus) {
    ...
    DWeb(opts.dir, opts, function (err, dweb) {
      if (err && err.name === 'MissingError') return bus.emit('exit:warn', 'Sorry, could not find a dWeb archive in this directory.')
