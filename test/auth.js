...
authServer(port, function (err, server, closeServer) {
  ...
  test('auth - create dWeb archive to publish', function (t) {
    rimraf.sync(path.join(fixtures, '.dweb'))
  ...
  test('auth - publish our awesome dWeb archive', function (t) {
  ...
  test('auth - publish our awesome dWeb archive with bad dweb.json url', function (t) {
  ...
  test('auth - publish our awesome dWeb archive without a dweb.json file', function (t) {
  ...
  test('auth - bad clone from registry', function (t) {
    ...
    var st = spawn(t, cmd, { cwd: baseDir })
    st.stderr.match(function (output) {
      t.same(output.trim(), 'DWeb with that name not found.', 'not found')
      st.kill()
      return true
    })
    st.stdout.empty()
